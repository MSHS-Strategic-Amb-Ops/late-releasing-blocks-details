...
conn2 <- dbConnect(odbc(), 
                  ...
                  uid = "kweons01" , 
                  pwd = "kweons01123$")
conn3 <- dbPool(drv = odbc(), dsn = "OAO Cloud DB Staging", timeout = 30)
...
temp_slot_tbl <-  tbl(conn3, "AMBULATORY_SLOT_TABLE_TEMP")
...
no_show_summary <- no_show %>%
  ...
  pivot_wider(names_from = block_type,
              ...
              values_fill = 0) %>%
  mutate(no_show_all = no_show_other + no_show_late_release + no_show_non_late_release_new,
         SameDay_Canceled_all = SameDay_Canceled_other + SameDay_Canceled_late_release + SameDay_Canceled_non_late_release_new,
         Arrived_all = Arrived_other + Arrived_late_release + Arrived_non_late_release_new) %>%
  mutate(no_show_rate_all = no_show_all/(Arrived_all + no_show_all),
         no_show_plus_rate_all = (no_show_all + SameDay_Canceled_all)/ (Arrived_all + no_show_all + SameDay_Canceled_all)) %>%
...
reactable(
  ...
  columns = list(
    ...
    no_show_plus_rate_all = colDef(
      name = "Overall No Show %",
      maxWidth = 150,
     headerStyle = list(background = "#7030a0", color = "white", fontWeight = "Bold", fontSize = "14px"),
      aggregate = JS("function(values, rows) {
        let totalArrived_all = 0
        let totalSameDay_Canceled_all = 0
        let totalno_show_all = 0
        rows.forEach(function(row) {
          totalArrived_all += row['Arrived_all']
          totalSameDay_Canceled_all += row['SameDay_Canceled_all']
          totalno_show_all += row['no_show_all']
        })
        return (totalSameDay_Canceled_all + totalno_show_all) / (totalSameDay_Canceled_all + totalno_show_all + totalArrived_all) || '-'
      }"),
      format = colFormat(percent = TRUE, digits = 0),
      style = list(borderLeft = "1.5px solid rgb(230, 230, 230)")
    ),
